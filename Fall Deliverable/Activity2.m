...
for k = 1 : nFrames
    ...
    %frameImage(~frameBW) = 0;
    framegray = rgb2gray(frameImage);
    filteredImage = wiener2(framegray,[10 10]);
    frameBW = bwareaopen(frameImage, 400);
    framegray = im2bw(framegray, .5);
    I2 = imdilate(frameImage,squareElem);
    I3 = imdilate(I2, squareElem);
    Ibwopen = imopen(framegray, squareElem);
    hBlob = vision.BlobAnalysis('MinimumBlobArea',100,'MaximumBlobArea',5000);
